...
export class DataStore {
  ...
  get visibleQuizzes(): Quiz[] {
    if (authStore.isAdmin || !authStore.isHost) return this.activeQuizzes;
    const ownerId = authStore.ownerId;
    return this.activeQuizzes.filter(q => q.hostId === ownerId);
  }

  get visibleResults(): QuizResult[] {
    if (authStore.isAdmin) return this.activeResults;
    if (authStore.isHost) {
      const quizIds = new Set(this.visibleQuizzes.map(q => q.id));
      return this.activeResults.filter(r => quizIds.has(r.quizId));
    }
    return this.activeResults;
  }

  get filteredQuizzes(): Quiz[] {
    let r = this.visibleQuizzes;
  ...
  get filteredResults(): QuizResult[] {
    let r = this.visibleResults;
    if (this.filters.quizId) r = r.filter(res => res.quizId === this.filters.quizId);
    return r;
  }

  get totalParticipants(): number { return this.visibleResults.length; }
  get averageScore(): number { return this.visibleResults.length ? Math.round(this.visibleResults.reduce((s, r) => s + r.percentage, 0) / this.visibleResults.length) : 0; }
  ...
  createQuiz = async (data: QuizFormData): Promise<Quiz | null> => {
    if (!authStore.canCreateQuizzes()) return null;
    const now = new Date().toISOString();
    const q: Quiz = {
      id: uuidv4(),
      ...data,
      questionsCount: 0,
      status: 'draft',
      hostId: authStore.isAdmin ? undefined : authStore.ownerId,
      isActive: true,
      createdAt: now,
      updatedAt: now,
    };
    try { await FirebaseService.setData(`quizzes/${q.id}`, q); runInAction(() => { this.quizzes.push(q); }); return q; } catch { return null; }
  };
  updateQuiz = async (id: string, data: Partial<QuizFormData>): Promise<boolean> => {
    if (!authStore.canManageQuizzes()) return false;
    const i = this.quizzes.findIndex(q => q.id === id);
    if (i === -1 || !authStore.ownsQuiz(this.quizzes[i])) return false;
    const u = { ...this.quizzes[i], ...data, updatedAt: new Date().toISOString() };
    try { await FirebaseService.setData(`quizzes/${id}`, u); runInAction(() => { this.quizzes[i] = u; }); return true; } catch { return false; }
  };
  deleteQuiz = async (id: string): Promise<boolean> => {
    if (!authStore.canManageQuizzes()) return false;
    const i = this.quizzes.findIndex(q => q.id === id);
    if (i === -1 || !authStore.ownsQuiz(this.quizzes[i])) return false;
    try { await FirebaseService.updateData(`quizzes/${id}`, { isActive: false }); runInAction(() => { this.quizzes[i].isActive = false; }); return true; } catch { return false; }
  };
  ...
  changeQuizStatus = async (id: string, status: 'draft' | 'active' | 'completed'): Promise<boolean> => {
    if (!authStore.isHost) return false;
    const i = this.quizzes.findIndex(q => q.id === id);
    if (i === -1 || !authStore.ownsQuiz(this.quizzes[i])) return false;
    const now = new Date().toISOString();
    try {
      ...
      runInAction(() => {
        this.quizzes[i] = { ...this.quizzes[i], status, updatedAt: now };
      });
      return true;
    } catch { return false; }
  };
  ...
  saveQuestionsForQuiz = async (quizId: string, questionsData: QuestionFormData[]): Promise<boolean> => {
    ...
    if (!quiz || !authStore.ownsQuiz(quiz)) return false;
    try {
      ...
      if (qi !== -1) {
        ...
        runInAction(() => { this.quizzes[qi] = { ...this.quizzes[qi], questionsCount: count, updatedAt: now }; });
      }
      return true;
    } catch { return false; }
  };
